...
namespace Skylark
{
    public class UIRoot : MonoBehaviour
    {
        ...
        private void Awake()
        {
            ...
            if (TopRoot == null)
            {
                TopRoot = transform.Find("Canvas/TopRoot");
                AbstractPanel[] panels = TopRoot.GetComponentsInChildren<AbstractPanel>();
                if (panels != null)
                {
                    for (int i = 0; i < panels.Length; i++)
                    {
